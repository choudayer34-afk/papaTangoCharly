...
export function renderPriorisation(container) {
  ...
  container.innerHTML = `
    ...
    <div class="view">
      ...
      <div class="chip-row" id="prio-filters" style="flex-wrap:wrap;align-items:center;">
        <span style="font-size:var(--font-size-sm);color:var(--color-text-muted);">Casquette</span>
        <span id="prio-hat-info"></span>
        <div class="chip-row" id="prio-hat-filter" style="margin-bottom:0;"></div>
        <button type="button" id="prio-weights-btn" class="btn btn-ghost btn-sm" style="margin-left:auto;">⚙️ Régler les poids</button>
      </div>
    ...
    const points = top
      .map((r, idx) => {
        ...
        return `
          ...
          ${idx < 3 ? `<text x="${cx.toFixed(1)}" y="${(cy + 4).toFixed(1)}" text-anchor="middle" font-size="11" fill="var(--color-text-inverse, #fff)" font-weight="700">${idx + 1}</text>` : ""}
        `;
      })
      .join("");

    quadrantEl.innerHTML = `
      <svg viewBox="0 0 ${W} ${H}" style="width:100%;height:auto;display:block;" role="img" aria-label="Matrice urgence-impact">
      ...
      <div class="fine-print" style="color:var(--color-text-muted);font-size:var(--font-size-xs);margin-top:4px;">Taille = blocage (bulle plus grosse si 🔴 Bloqué) · couleur = rang parmi les 3 premiers.</div>
    `;
  }
