...
Statut (21/09/2026, LOT 6) : **Partiellement terminé — un des deux liens posé, l'autre en attente d'arbitrage.** Ce TODO comporte deux liens distincts (voir "Solution" ci-dessus et USE-UX-026) :

1. **Entité créée ↔ projet de contexte, quand déjà connu à la création** (Task/FollowUp/Meeting/Decision — jamais Project, qui ne se lie pas à lui-même, ni Resource, qui n'a pas de `projectId` à sa création) : **fait**. `js/domain/inbox.js#qualify()` appelle désormais `linkEntityToProjectIfKnown()` (nouvelle fonction locale à ce fichier, table `OUTCOME_PROJECT_LINK`) juste après avoir journalisé la qualification, en réutilisant `linksApi.createLink` — exactement le mécanisme déjà écrit pour « + Créer et lier » (`js/components/linkedItems.js#openCreateFormFor`), aucune nouvelle architecture de gestion des liens. Pour "task", l'entité vient d'être créée dans la même fonction (son `projectId` est déjà en mémoire) ; pour "followup"/"meeting"/"decision", l'entité déjà créée côté vue est refetchée par son id (`extra.id`) pour lire son `projectId`. Aucun lien n'est posé quand `projectId` est absent, ou quand l'entité/le projet référencé n'existe plus.

2. **InboxItem source ↔ entité créée** (les six entités : Task, FollowUp, Project, Meeting, Decision, Resource) : **NON fait — bloqué par une ambiguïté architecturale signalée à Charles-Henri avant tout code, conformément à la règle d'arbitrage.** `js/components/linkedItems.js#resolveRef`/`resolveRefDirect` ne savent résoudre un lien pointant vers un InboxItem que via le type de référence `"Kept"`, qui filtre explicitement sur `status === "kept"` (comportement délibéré, lié à l'auto-archivage à 15 jours — voir le commentaire de `resolveRefDirect`, cas `"Kept"`). Or les six issues concernées par ce point posent `status: "processed"`, jamais `"kept"` — un lien créé tel quel s'afficherait "Élément supprimé" dès sa création, de façon permanente. Corriger ce point demanderait de toucher `js/components/linkedItems.js`, qui ne fait PAS partie des "Fichiers concernés" déclarés par ce TODO, et pourrait entrer en tension avec la consigne "ne pas créer de nouvelle architecture de gestion des liens". Point signalé à Charles-Henri (ambiguïté d'architecture/produit, pas un détail d'implémentation) avant toute écriture de code sur ce point précis — en attente d'arbitrage, non traité unilatéralement.

TODO-008 reste donc **partiellement terminé** tant que ce second point n'a pas été arbitré. Couvert par un test écrit (`tests/unit/lot6-inbox-qualify-project-links.spec.js`, TEST-022 étendu) pour le seul point 1 — voir sa section 2 (Tests) en tête de fichier pour le détail des scénarios ; non exécuté dans cet environnement (registre npm bloqué), à confirmer par un passage réel du workflow GitHub Actions.
...
**Statut (21/09/2026)** : **Partiellement terminé — non clos.** TODO-013 est **Terminé** (implémentation complète, tests écrits — voir TODO-013 en section 5). TODO-008 est **partiellement terminé** : le lien entité créée ↔ projet est fait, mais le lien InboxItem source ↔ entité créée reste bloqué par une ambiguïté d'architecture signalée à Charles-Henri avant toute écriture de code sur ce point précis (voir TODO-008 en section 5 pour le détail complet et les options proposées) — en attente de son arbitrage. Par ailleurs, comme pour tous les lots précédents, les tests écrits (`tests/unit/lot6-inbox-qualify-project-links.spec.js`, `tests/e2e/lot6-inbox-bulk-qualification.spec.js`) n'ont pas pu être exécutés dans cet environnement (registre npm bloqué) et restent à confirmer par un passage réel du workflow GitHub Actions. **Le LOT 6 n'est donc pas considéré comme terminé** tant que ces deux conditions ne sont pas réunies : (1) arbitrage de Charles-Henri sur le point 1 de TODO-008, (2) confirmation CI des deux nouveaux fichiers de test. Il n'enchaîne pas automatiquement sur le LOT 7.
