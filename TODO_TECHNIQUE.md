...
2. **Cohérence de la liaison Ressource↔Tâche/Projet (COMP-UX-014)** : confirmé par inspection (`js/views/kanban.js`, `js/views/projects.js`) que deux mécanismes coexistent bien avec des résultats différents — le bloc dédié "📎 Ressources" d'une fiche Tâche/Projet (boutons "+ Nouvelle ressource"/"🔗 Lier existante") peuple `taskIds`/`projectIds` sur la Ressource (`resourcesApi.linkToTask`/`linkToProject`) sans jamais créer de lien générique ; le chemin universel "🔗 Lié" → "+ Créer et lier" (`js/components/linkedItems.js#openCreateFormFor`) crée un lien générique (`linksApi.createLink`) sans jamais peupler `taskIds`/`projectIds`. Une Ressource créée par l'un des deux chemins n'apparaissait donc que dans une seule des deux sections de la fiche d'origine. Corrigé exactement comme demandé par la Solution ("transmettre systématiquement l'identifiant de la fiche d'origine dans le chemin « + Créer et lier »") : le cas `"Resource"` de `openCreateFormFor` transmet désormais `taskId`/`projectId` (déduit de `ref.type`/`ref.id`) à `openCreateResourceModal`, qui sait déjà lire ces deux champs pour peupler `taskIds`/`projectIds` à la création (mécanisme réutilisé tel quel, déjà utilisé par "+ Nouvelle ressource" — aucune modification de `js/domain/resources.js` ni `js/views/resources.js` nécessaire). Le lien générique existant est conservé sans changement. **Conséquence assumée, à noter** : une Ressource créée via "+ Créer et lier" apparaît désormais À LA FOIS dans "📎 Ressources" et dans "🔗 Lié" de la fiche d'origine (les deux mécanismes sont désormais alimentés ensemble, pas fusionnés) — la Solution actée ne demandait que la transmission systématique de l'id d'origine, pas une fusion des deux mécanismes d'affichage ; ce doublon visuel est un effet secondaire visible de l'application littérale de la solution demandée, signalé ici plutôt que corrigé unilatéralement (une fusion serait un changement de comportement plus large, hors périmètre déclaré de ce TODO).

Aucun test automatisé écrit, conformément à la roadmap ("Validation : Non déterminé — test manuel des deux chemins de liaison") : vérification faite par inspection de code (lecture complète de `js/domain/resources.js`, `js/views/resources.js`, `js/views/kanban.js`, `js/views/projects.js`, `js/components/linkedItems.js`) plutôt que par exécution, l'environnement de rédaction ne pouvant de toute façon pas lancer Playwright (registre npm bloqué). Aucune régression identifiable sur le lien générique existant (chemin inchangé pour tout `ref.type` autre que "Task"/"Project", ex. lier une Ressource depuis une fiche Suivi/Réunion/Décision) ni sur les mécanismes `linkToTask`/`linkToProject` déjà utilisés ailleurs. **Ce TODO ne sera considéré comme définitivement clos qu'après le test manuel des deux chemins de liaison par Charles-Henri**, prévu comme seule Validation de ce TODO — en particulier le doublement d'affichage signalé plus haut (Ressource visible dans "📎 Ressources" ET "🔗 Lié" après un "+ Créer et lier"), qu'il conviendra de juger acceptable ou non en usage réel.
...
1. **Filtres Type/Projet/Personne au-dessus de la grille** : réutilise tel quel le menu "🔧 Filtrer" déjà écrit pour Pilotage/Projets (`.filter-popover`/`.filter-popover-panel`, voir `js/views/kanban.js`) plutôt que d'inventer un nouveau composant — un menu replié par défaut contenant un filtre Type (4 chips à bascule, ✅ Tâches/🗓️ Réunions/🗳️ Décisions/👀 Suivis, tous actifs par défaut = comportement actuel inchangé tant qu'on n'y touche pas), un filtre Projet (`<select>`, projets fermés exclus comme le reste du Calendrier) et un filtre Personne (`<select>`, liste `peopleApi.sortPeople`). Le filtre Personne ne peut en pratique s'appliquer qu'aux Suivis (👀) : c'est le seul des 4 types à porter réellement un `personId` renseigné via une UI existante — la Tâche n'a jamais de personne assignée (règle déjà actée, voir `openTaskDetail`), et le champ `participants` d'une Réunion existe dans le modèle (`js/domain/meetings.js`) mais n'est exposé par aucun formulaire de création à ce jour. Sélectionner une personne masque donc Tâches/Réunions/Décisions ce jour-là (documenté en commentaire dans le code) plutôt que de laisser croire à un filtrage qui ne peut jamais trouver de correspondance sur ces trois types.
