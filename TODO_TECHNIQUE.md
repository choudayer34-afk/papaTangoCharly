...
Aucun test automatisé écrit, conformément à la roadmap ("Validation : Non déterminé — test manuel des deux chemins de liaison") : vérification faite par inspection de code (lecture complète de `js/domain/resources.js`, `js/views/resources.js`, `js/views/kanban.js`, `js/views/projects.js`, `js/components/linkedItems.js`) plutôt que par exécution, l'environnement de rédaction ne pouvant de toute façon pas lancer Playwright (registre npm bloqué). Aucune régression identifiable : le chemin "+ Créer et lier" pour une Ressource se comporte exactement comme avant ce lot. **Ce TODO reste ouvert** : la partie URL (COMP-UX-012) était déjà résolue par TODO-006 (voir point 1 ci-dessus), et la partie liaison (COMP-UX-014) est désormais explicitement **non traitée par choix produit** — à ne pas rouvrir sans nouvelle décision de Charles-Henri. Une vérification manuelle de Charles-Henri sur le chemin "+ Créer et lier" (comportement inchangé) reste néanmoins utile pour confirmer l'absence de régression.
...
Aucun test automatisé écrit, conformément à la roadmap ("Tests nécessaires : Non déterminé") — vérification faite par inspection de code complète des fichiers concernés, l'environnement de rédaction ne pouvant de toute façon pas exécuter l'app ni Playwright (registre npm bloqué). **Le LOT 7 n'est donc pas considéré comme définitivement clos** tant que Charles-Henri n'a pas testé manuellement les deux écrans concernés. Il n'enchaîne pas automatiquement sur le LOT 8.
