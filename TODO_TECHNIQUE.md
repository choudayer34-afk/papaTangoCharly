...
## [x] P1 — TODO-008 — Introduire un lien automatique lors de la qualification Inbox — **Terminé**
...
Statut (21/09/2026, LOT 6) : **Terminé (implémentation) — les deux liens sont posés, sous réserve de confirmation CI.** Ce TODO comporte deux liens distincts (voir "Solution" ci-dessus et USE-UX-026) :

1. **Entité créée ↔ projet de contexte, quand déjà connu à la création** (Task/FollowUp/Meeting/Decision — jamais Project, qui ne se lie pas à lui-même, ni Resource, qui n'a pas de `projectId` à sa création) : fait. `js/domain/inbox.js#qualify()` appelle `linkEntityToProjectIfKnown()` juste après avoir journalisé la qualification, en réutilisant `linksApi.createLink` — exactement le mécanisme déjà écrit pour « + Créer et lier » (`js/components/linkedItems.js#openCreateFormFor`), aucune nouvelle architecture de gestion des liens. Pour "task", l'entité vient d'être créée dans la même fonction (son `projectId` est déjà en mémoire) ; pour "followup"/"meeting"/"decision", l'entité déjà créée côté vue est refetchée par son id (`extra.id`) pour lire son `projectId`. Aucun lien n'est posé quand `projectId` est absent, ou quand l'entité/le projet référencé n'existe plus.

2. **InboxItem source ↔ entité créée** (les six entités : Task, FollowUp, Project, Meeting, Decision, Resource) : ce point était initialement bloqué par une ambiguïté architecturale, signalée à Charles-Henri avant tout code, conformément à la règle d'arbitrage — `js/components/linkedItems.js#resolveRef`/`resolveRefDirect` ne savaient résoudre un lien pointant vers un InboxItem que via le type de référence `"Kept"`, filtré sur `status === "kept"`, alors que ces six issues posent `status: "processed"`. **Arbitrage de Charles-Henri (21/09/2026)** : ajouter un type de référence dédié `InboxSource`, résolu directement par l'id de l'InboxItem, sans exiger `status === "kept"`, sans toucher à la sémantique existante de `"Kept"`, sans transformer un InboxItem `processed` en `kept`, et sans modifier la politique de conservation/l'auto-archivage à 15 jours — en autorisant explicitement, pour ce seul besoin, la modification de `js/components/linkedItems.js` (hors des "Fichiers concernés" initialement déclarés, mais nécessaire à la réalisation de ce point, donc pas une extension de périmètre). **Réalisé en conséquence** : `js/domain/inbox.js#qualify()` pose désormais aussi, pour les six issues, le lien `{entité} ↔ {InboxSource, id: <InboxItem>}` via une nouvelle table `OUTCOME_ENTITY` et la fonction `linkEntityToInboxSource()` (même mécanisme `linksApi.createLink`, aucune nouvelle architecture de liens) ; `js/components/linkedItems.js#resolveRefDirect` reçoit un nouveau cas `"InboxSource"` qui résout l'InboxItem par `inboxApi.getInboxItem(ref.id)` sans filtre de statut ; le cas `"Kept"` existant, `resolveRef`/`allRefs` (utilisés uniquement par "🔗 Lier une fiche"/"+ Créer et lier", jamais concernés par ce nouveau type, posé uniquement automatiquement) et le balayage `autoArchiveStaleKept()` restent inchangés. Le clic sur ce nouveau lien ouvre une fiche dédiée en lecture seule, `js/views/inbox.js#openInboxSourceDetail` (texte capturé + date), volontairement distincte de `openKeptItemDetail` : cette dernière expose des actions de gestion ("🗄️ Archiver", "🔁 Changer de type") qui n'ont pas de sens pour un InboxItem déjà `processed` et l'aurait, de plus, affiché avec un intitulé "🧠 Information" incorrect.

TODO-008 est donc désormais **terminé au niveau de l'implémentation** pour ses deux points. Couvert par trois fichiers de test (TEST-022 étendu) : `tests/unit/lot6-inbox-qualify-project-links.spec.js` (création des deux liens pour les 6 entités côté domaine, absence de lien projet pour Resource/quand `projectId` est inconnu, régression des 9 issues de qualification) et `tests/e2e/lot6-inbox-source-link.spec.js` (résolution réelle du lien InboxSource après qualification en Tâche — pas "Élément supprimé", fiche dédiée en lecture seule sans action de gestion — et non-régression du type `"Kept"` existant, exercé via un lien manuel indépendant).

**Premier passage réel du workflow GitHub Actions (21/09/2026)** : 39/42 tests verts, 3 échecs — tous trois des défauts des NOUVEAUX tests de ce lot, pas du code applicatif :
- `tests/unit/lot6-inbox-qualify-project-links.spec.js` — "Aucun projectId connu à la création → aucun lien entité↔projet créé" : le test vérifiait "aucun lien du tout" au lieu de "aucun lien vers `Project`" spécifiquement — devenu un faux-négatif dès que le lien InboxSource (toujours posé, lui, que le projet soit connu ou non) a existé. Corrigé en ciblant explicitement le type `Project` du côté opposé, et en vérifiant en plus que le lien InboxSource, lui, est bien présent.
- `tests/e2e/lot6-inbox-source-link.spec.js` (les deux tests) — `page.getByText(...)` ambigu (« strict mode violation ») : `openInboxSourceDetail` affichait "📥 Capture d'origine" à la fois comme titre de modale ET comme label de champ dans son corps ; `openKeptItemDetail` (code préexistant, non touché par ce lot) affiche de la même façon "🧠 Information"/"💡 Idée" à la fois en titre et en label. Corrigé : le label du corps de `openInboxSourceDetail` est renommé en "Texte capturé" (petite clarification de copie, aucun changement de comportement) ; les deux tests ciblent désormais `getByRole("heading", ...)` plutôt que `getByText(...)` pour ne viser que le titre de la modale.

**Deuxième passage réel du workflow GitHub Actions (21/09/2026, après les corrections ci-dessus)** : 40/42 tests verts, 2 échecs — tous deux dans `tests/e2e/lot6-inbox-source-link.spec.js`, et tous deux des défauts des tests, pas du code applicatif (confirmé lien par lien avec `js/views/kanban.js`) :
- Le premier test échouait sur `await expect(sourceRow).toBeVisible(...)` avec `Received: hidden`, alors que l'élément était bien résolu dans le DOM à chaque tentative (24 relances sur ~10s). Cause identifiée : la fiche Tâche est une fiche à onglets (vague 24, préexistante, non touchée par ce lot) — sa section "🔗 Lié" (`#detail-links`) est regroupée sous l'onglet "Activité" (`js/views/kanban.js`, `.fiche-tabpanel[data-tabpanel="activity"]`), qui porte l'attribut `hidden` tant que cet onglet n'a pas été sélectionné (l'onglet "Détails" est actif par défaut à l'ouverture). Le test avait été rédigé sans tenir compte de cette structure à onglets. Corrigé côté test uniquement : un clic sur l'onglet "Activité" (`page.getByRole("tab", { name: "Activité" })`) est ajouté avant toute assertion ou interaction touchant `#detail-links`/`#link-existing-btn` — y compris une seconde fois après la réouverture de la fiche par `onLinked` dans le second test, celle-ci recréant un HTML frais où l'onglet "Détails" redevient actif par défaut.
- Le second test échouait sur `page.getByRole("button", { name: "Fermer" }).click()` avec une violation de strict mode Playwright : ce sélecteur matchait à la fois le bouton "Fermer" attendu et un bouton sans rapport, "✕" avec `aria-label="Fermer ce conseil"` (bandeau d'astuce par ailleurs présent sur la page), `getByRole` matchant par sous-chaîne par défaut. Corrigé en ajoutant `exact: true` à ce sélecteur.

Aucune modification de code applicatif n'a été nécessaire pour ces deux corrections : les deux causes sont des défauts de rédaction des tests E2E de ce lot face à un comportement d'interface préexistant (fiche à onglets) et à une ambiguïté de sélecteur Playwright non liée à TODO-008.

**Troisième passage réel du workflow GitHub Actions (21/09/2026, après les corrections ci-dessus)** : 41/42 tests verts, 1 échec — la même ambiguïté de sélecteur `getByRole("button", { name: "Fermer" })` que celle déjà corrigée au round précédent (violation de strict mode avec le bouton sans rapport "Fermer ce conseil"), mais dans le PREMIER test cette fois (`await expect(page.getByRole("button", { name: "Fermer" })).toBeVisible();`, sur la fiche `openInboxSourceDetail`) — le correctif `exact: true` n'avait été appliqué qu'au sélecteur identique du second test, cette occurrence-ci ayant été omise par erreur. Corrigée de la même façon (`exact: true`). Aucune modification de code applicatif.

**Quatrième passage réel du workflow GitHub Actions (21/09/2026, après cette dernière correction)** : **42/42 tests verts, confirmé par Charles-Henri.** TODO-008 est désormais **définitivement clos**, ses deux points (lien entité↔projet, lien InboxSource↔entité) validés en conditions réelles (Firebase emulators + Playwright), sans régression sur les tests des lots précédents.
...
Statut (21/09/2026, LOT 5) : **Terminé.** Deux sections ajoutées à `PROJECT_CONTEXT.md` (aucun fichier applicatif touché, conformément à "Fonctions concernées : aucune") : §16 « Évolution du schéma des données », qui documente (16.1) le critère de choix entre tableau embarqué et collection dédiée à partir des exemples déjà en place dans le code (`notesLog`/`checklist`/`steps`/`entries`/`parts` embarqués vs `history`/`links`/`tags` dédiées), en signalant explicitement l'incohérence connue mais non résolue par cette section (DATA-002 : `notesLog` suit la logique inverse de `history`/`links` sans justification documentée — non tranchée rétroactivement ici, hors périmètre de ce TODO) ; et (16.2) les trois stratégies d'évolution de schéma déjà en usage (valeur par défaut à la lecture, migration ponctuelle à drapeau sur `preferences`, normalisation à chaque lecture jamais réécrite), chacune avec son exemple réel et son cas d'usage, en signalant l'angle mort déjà connu de la 3ᵉ stratégie (DATA-008/DATA-009 : `exportAllUserData()` contourne la normalisation de `followups.js`, backlog sans TODO dédié, non corrigé ici). §17 « Schéma des collections (référence) » ajoute le tableau récapitulatif demandé par DATA-015 : les 14 collections Firestore réelles de l'app (`tasks`, `projects`, `people`, `followUps`, `resources`, `meetings`, `decisions`, `objectives`, `prompts`, `inboxItems`, `links`, `tags`, `preferences`, `history`), une table par collection (champ, type, optionalité, note), établies par inspection directe de chaque `js/domain/*.js` (fonctions `createXxx()`/`withDefaults()`) plutôt que reconstituées de mémoire ; les champs communs `id`/`createdAt`/`updatedAt` (posés par `storage.put()`) sont factorisés une seule fois en tête de section pour ne pas les répéter 14 fois. `casquettes.js`/`templates.js` (données statiques, aucune collection Firestore) et `priorisation.js`/`workload.js`/`projectHealth.js`/`convert.js` (logique calculée sur d'autres collections, aucune collection propre) ont été vérifiés puis délibérément exclus du tableau — confirmé par inspection (`grep` de `COLLECTION`/`storage.` dans chacun) plutôt que supposé. Le volet vocabulaire (glossaire objet ↔ libellé), un temps envisagé sous ce même TODO, reste hors périmètre : retiré de TODO-012 le 15/09/2026 et porté exclusivement par TODO-021 (non planifié à ce jour), conformément à la note en tête de ce TODO — non traité par ce lot. Aucun test automatisé prévu ni applicable (documentation pure, "Validation : relecture croisée" — la relecture croisée reste à faire par Charles-Henri).

## [x] P2 — TODO-013 — Option de qualification Inbox en lot pour les cas simples — **Terminé**
...
Statut (21/09/2026, LOT 6) : **Terminé.** Option « Traiter en lot » ajoutée dans `js/views/inbox.js` (bouton "☑️ Traiter en lot" dans le topbar de l'Inbox, désactivé/replié par défaut — aucune case à cocher tant qu'il n'est pas activé explicitement) et, séparément, dans `js/components/weeklyReview.js` (même bouton, propre à la section "📥 Inbox" de la Revue hebdomadaire uniquement). Strictement limitée aux 3 qualifications sans formulaire de création — Information (`kept`), Idée (`idea`), Archivé (`archived`) — dérivées par filtre de la liste déjà existante (`QUALIFY_CHOICES` dans `js/views/inbox.js`, `WR_BULK_CHOICES` local à `weeklyReview.js`) plutôt que redéfinies séparément, pour ne jamais pouvoir diverger silencieusement de la liste "sans formulaire" ; Action/Suivi/Projet/Réunion/Décision/Ressource ne sont jamais proposées en lot et gardent leur formulaire dédié, inchangé. Comme demandé, aucun mécanisme générique de sélection/traitement multiple n'a été créé : chaque fichier porte son propre état local indépendant (un booléen `bulkMode` + un `Set` d'ids), sans rien de partagé entre `inbox.js`, `weeklyReview.js` et le mécanisme de sélection en masse déjà existant par ailleurs pour les tâches (`js/views/kanban.js`) — seule la classe CSS `.pilotage-bulk-toolbar` de ce dernier est réutilisée, pour la seule cohérence visuelle. Garde anti-double-traitement : la sélection est vidée et la barre d'actions/les cases à cocher redessinées de façon synchrone AVANT le `await` de la qualification elle-même, si bien qu'un second clic (ou un clic pendant l'attente) ne retrouve plus ni les ids déjà envoyés ni les boutons qui les ont envoyés. Le comportement individuel existant (bouton "Traiter" → les 9 choix) reste intégralement inchangé et disponible, y compris quand le mode lot est actif. Dans la Revue hebdomadaire, un traitement en lot ne ferme PAS toute la revue (contrairement au clic individuel, qui la ferme pour ouvrir la fiche résultante) : seule la section Inbox se met à jour sur place, `renderRows()` (les 7 autres sections) restant totalement inchangée. Couvert par un test E2E écrit (`tests/e2e/lot6-inbox-bulk-qualification.spec.js`, TEST-022 étendu) : mode désactivé par défaut, seules les 3 issues autorisées sont proposées en lot, qualification effective sans double traitement, comportement individuel préservé, et comportement cohérent de la Revue hebdomadaire (revue non fermée). **Confirmé par un passage réel du workflow GitHub Actions** : les 3 tests sont passés au tout premier passage CI et sont restés verts à chaque passage suivant, jusqu'à la confirmation finale (42/42) de Charles-Henri le 21/09/2026. TODO-013 est désormais **définitivement clos**.

## [x] P2 — TODO-014 — Cohérence de la liaison Ressources et validation d'URL — **Terminé**
...
Statut (21/09/2026, LOT 7) : **Terminé (implémentation) — en attente de vérification manuelle par Charles-Henri**, conformément à la Validation prévue ("Non déterminé — test manuel des deux chemins de liaison"). Précision sur son périmètre réel — ce TODO porte deux problèmes distincts (COMP-UX-012, COMP-UX-014) et l'un des deux s'est révélé déjà résolu par un autre chantier :

1. **Validation du format d'URL (COMP-UX-012)** : en inspectant `js/views/resources.js` avant toute modification, ce point s'est avéré **déjà entièrement réalisé** — par TODO-006 (LOT 1), qui liste lui-même COMP-UX-012 parmi les problèmes qu'il résout ("validation URL, même chantier de validation partagée"). `validateUrlField(bodyEl, "#res-url")`/`validateUrlField(bodyEl, "#res-detail-url")` sont déjà appliqués aux deux formulaires Ressource (création et fiche détail), confirmés verts en CI depuis le LOT 1 (`tests/e2e/lot1-form-validation.spec.js`, cas "Ressource : une URL mal formée est rejetée avec un message dédié (UX-012)"). **Aucune modification apportée sur ce point** — le refaire aurait été une duplication opportuniste d'un travail déjà livré et validé. Point signalé ici plutôt que traité en silence, conformément à la consigne de signaler tout constat hors périmètre attendu : la roadmap gagnerait à retirer COMP-UX-012 de la liste "Problèmes résolus" de TODO-014, puisqu'il ne l'est déjà plus par ce TODO mais par TODO-006.

2. **Cohérence de la liaison Ressource↔Tâche/Projet (COMP-UX-014)** : confirmé par inspection (`js/views/kanban.js`, `js/views/projects.js`) que deux mécanismes coexistent bien avec des résultats différents — le bloc dédié "📎 Ressources" d'une fiche Tâche/Projet (boutons "+ Nouvelle ressource"/"🔗 Lier existante") peuple `taskIds`/`projectIds` sur la Ressource (`resourcesApi.linkToTask`/`linkToProject`) sans jamais créer de lien générique ; le chemin universel "🔗 Lié" → "+ Créer et lier" (`js/components/linkedItems.js#openCreateFormFor`) crée un lien générique (`linksApi.createLink`) sans jamais peupler `taskIds`/`projectIds`. Une Ressource créée par l'un des deux chemins n'apparaît donc que dans une seule des deux sections de la fiche d'origine. Une première implémentation avait suivi la Solution à la lettre ("transmettre systématiquement l'identifiant de la fiche d'origine dans le chemin « + Créer et lier »") en faisant transmettre `taskId`/`projectId` par le cas `"Resource"` de `openCreateFormFor`, en plus du lien générique déjà créé — mais cela faisait apparaître la Ressource dans LES DEUX sections à la fois ("📎 Ressources" ET "🔗 Lié"), un doublon visuel signalé à Charles-Henri comme effet de bord plutôt que tranché unilatéralement. **Décision de Charles-Henri (21/09/2026)** : « je ne veux pas du doublon on garde uniquement le lié et pas ressource ». Le cas `"Resource"` de `openCreateFormFor` a donc été **remis dans son état d'origine** : il ne crée que le lien générique (`linksApi.createLink`), sans transmettre `taskId`/`projectId`. **COMP-UX-014 n'est donc pas résolu** — l'incohérence entre les deux mécanismes subsiste (une Ressource créée par l'un des deux chemins n'apparaît toujours que dans une seule des deux sections) — mais c'est un choix produit assumé par Charles-Henri, préférant cette incohérence au doublon visuel, et non un oubli.

Aucun test automatisé écrit, conformément à la roadmap ("Validation : Non déterminé — test manuel des deux chemins de liaison") : vérification faite par inspection de code (lecture complète de `js/domain/resources.js`, `js/views/resources.js`, `js/views/kanban.js`, `js/views/projects.js`, `js/components/linkedItems.js`) plutôt que par exécution, l'environnement de rédaction ne pouvant de toute façon pas lancer Playwright (registre npm bloqué). Aucune régression identifiable : le chemin "+ Créer et lier" pour une Ressource se comporte exactement comme avant ce lot. La partie URL (COMP-UX-012) était déjà résolue par TODO-006 (voir point 1 ci-dessus), et la partie liaison (COMP-UX-014) est explicitement **non traitée par choix produit** de Charles-Henri (« je ne veux pas du doublon on garde uniquement le lié et pas ressource ») — à ne pas rouvrir sans nouvelle décision de sa part.

**Clôture (21/09/2026)** : **confirmé par Charles-Henri** (« cloture le lot 7 »). Ce TODO est **définitivement clos** au niveau prévu par sa Validation ("test manuel") : la partie URL était déjà résolue par TODO-006, et la partie liaison est intentionnellement laissée en l'état par décision produit assumée — il n'y a donc plus rien en suspens sur ce TODO.

## [x] P2 — TODO-015 — Calendrier : filtre, création directe, cohérence d'affichage — **Terminé**
...
Statut (21/09/2026, LOT 7) : **Terminé (implémentation) — en attente de vérification manuelle par Charles-Henri**, conformément à la Validation prévue ("Non déterminé — pas de test automatisé prévu"). Les trois volets sont implémentés dans `js/views/calendar.js`, plus un ajout mineur dans `styles/components.css` (non listé dans les "Fichiers concernés" mais indispensable à l'affichage des deux premiers volets — filtres et cohérence de troncature sont par nature autant CSS que JS) :

1. **Filtres Type/Projet au-dessus de la grille** : réutilise tel quel le menu "🔧 Filtrer" déjà écrit pour Pilotage/Projets (`.filter-popover`/`.filter-popover-panel`, voir `js/views/kanban.js`) plutôt que d'inventer un nouveau composant — un menu replié par défaut contenant un filtre Type (4 chips à bascule, ✅ Tâches/🗓️ Réunions/🗳️ Décisions/👀 Suivis, tous actifs par défaut = comportement actuel inchangé tant qu'on n'y touche pas) et un filtre Projet (`<select>`, projets fermés exclus comme le reste du Calendrier). Un filtre Personne (`<select>`, liste `peopleApi.sortPeople`) avait initialement été ajouté avec les deux autres, mais retiré avant livraison finale — voir le point sur la décision de Charles-Henri ci-dessous.
2. **Point de création rapide sur un jour** : ajout de deux actions "+ Tâche"/"+ Réunion" dans la fiche `openDayAgenda` (celle qui s'ouvre déjà au clic sur un jour), chacune ouvrant le formulaire de création existant correspondant (`openCreateTaskModal`/`openCreateMeetingModal`, réutilisés tels quels comme demandé par les "Dépendances") avec la date de ce jour déjà pré-remplie (`dueDate`/`date`). Aucun nouveau formulaire créé. Le rafraîchissement de la grille après création est automatique (abonnements `tasksApi.subscribe`/`meetingsApi.subscribe` déjà en place), aucune modification nécessaire de ce côté.
3. **Cohérence de troncature Mois/Semaine** : la vue Mois tronquait déjà chaque titre sur une seule ligne (`.cal-pill`, `text-overflow: ellipsis`) tout en limitant l'affichage à 3 éléments par jour (`+N`) ; la vue Semaine, elle, réutilisait `.item-row`/`.item-title` génériques sans aucune règle de troncature — un titre long y passait normalement à la ligne, sans limite de nombre d'éléments. Harmonisé en appliquant la même troncature à une ligne (ellipsis) aux titres de la vue Semaine, via une règle CSS scopée à `.cal-week-day .item-title` pour ne toucher aucun autre usage de `.item-title` ailleurs dans l'app. La limite "3 éléments + `+N`" de la vue Mois, propre à l'espace restreint d'une cellule de grille, n'a pas été étendue à la vue Semaine (qui dispose de bien plus de place par jour) : la Solution actée demandait d'« harmoniser la troncature », pas de dupliquer une contrainte de mise en page qui n'a de sens que dans un format compact.

**Filtre Personne retiré (21/09/2026)** : un filtre Personne avait été ajouté au point 1 ci-dessus, mais son utilité réelle était limitée — il ne pouvait en pratique s'appliquer qu'aux Suivis (👀), seul des 4 types à porter réellement un `personId` renseigné via une UI existante (la Tâche n'a jamais de personne assignée, règle déjà actée voir `openTaskDetail` ; le champ `participants` d'une Réunion existe dans le modèle `js/domain/meetings.js` mais n'est exposé par aucun formulaire de création). Ce point a été signalé à Charles-Henri comme limitation plutôt que masqué. **Décision de Charles-Henri (21/09/2026)** : « donc on retire le filtre dans calendrier ». Le filtre Personne a été **intégralement retiré** de `js/views/calendar.js` (import `peopleApi`, état, HTML, logique de filtrage, alimentation du `<select>`, écouteur d'événement, abonnement `peopleApi.subscribe` et son nettoyage) — seuls les filtres Type et Projet subsistent, ce qui constitue le périmètre final retenu pour les "filtres simples" demandés par ce TODO.

**Incident et correction (21/09/2026)** : la première livraison de `calendar.js` contenait une erreur de syntaxe en production, remontée par Charles-Henri ("Uncaught SyntaxError: Unexpected identifier 'personId' (at calendar.js:73:31)"), alors que la vérification locale (`node --check`) l'avait laissée passer. Cause racine identifiée : des caractères backtick utilisés dans des commentaires HTML (`<!-- -->`) situés À L'INTÉRIEUR du template literal JS (`` container.innerHTML = `...` ``) referment/rouvrent prématurément ce template literal, produisant un code syntaxiquement ambigu que `node --check` ne détecte pas de façon fiable sur ce cas précis (faux négatif confirmé par une reproduction minimale). Corrigé en réécrivant les commentaires concernés sans aucun backtick. **Vérification renforcée adoptée pour la suite** : `node --input-type=module --check < fichier.js` (au lieu de `node --check fichier.js`), qui détecte correctement cette classe d'erreur ; appliquée en repassage sur l'ensemble des fichiers JS touchés dans ce lot et le précédent, tous confirmés sains.

Aucun test automatisé écrit, conformément à la roadmap ("Validation : Non déterminé — pas de test automatisé prévu"). Vérifié par inspection de code (lecture complète de `js/views/calendar.js` avant et après modification, des CSS `.cal-pill`/`.item-title`/`.filter-popover-panel`, et des formulaires `openCreateTaskModal`/`openCreateMeetingModal` pour confirmer leur support de `dueDate`/`date` en pré-remplissage) plutôt que par exécution — l'environnement de rédaction ne peut de toute façon pas lancer l'app ni Playwright (registre npm bloqué). Aucune régression identifiable : les 4 types restent actifs par défaut (comportement d'affichage inchangé tant qu'aucun filtre n'est touché), le glisser-déposer et l'agenda du jour existants ne sont pas modifiés dans leur logique propre.
...
Statut (21/09/2026, LOT 8) : **Terminé (implémentation) — en attente de vérification manuelle par Charles-Henri**, conformément à la Validation prévue ("Non déterminé — vérification manuelle au clavier recommandée"). Les quatre volets sont implémentés, tous dans `js/components/modal.js` sauf le dernier :

1. **Piège de focus standard et restitution à la fermeture (COMP-UX-001/002)** : `openModal()` mémorise désormais l'élément qui avait le focus avant son ouverture (`document.activeElement`) et le lui restitue dans `close()` (si toujours présent dans le document). À l'ouverture, le focus est déplacé dans la modale — sur son premier élément focusable (champ ou bouton), ou sur la modale elle-même (`tabindex="-1"` ajouté comme repli) si elle n'en contient aucun — sans quoi le piège de focus ne servirait à rien tant que l'utilisateur n'a pas déjà tabulé une première fois dedans. `Tab`/`Shift+Tab` bouclent désormais sur le premier/dernier élément focusable de la modale plutôt que de continuer vers des éléments de la page en dessous (invisibles sous l'overlay mais jusqu'ici toujours dans l'ordre de tabulation). La liste des éléments focusables exclut ceux masqués (`offsetParent === null`, ce qui couvre notamment un champ caché sous un onglet non sélectionné — voir le bug de fiche à onglets déjà rencontré et corrigé au LOT 6 sous TODO-008) — et est recalculée à chaque `Tab` plutôt que figée à l'ouverture, pour rester correcte si le contenu de la modale change en cours de vie (ex. `attachProjectQuickCreate`, qui ajoute un champ après un choix).
2. **`Échap` aligné sur la protection du clic en dehors (dans le Problème, sans COMP-UX dédié)** : la logique de garde déjà existante pour le clic en dehors (ne pas fermer si la modale contient un champ modifiable — `input`/`textarea`/`select` ni `readonly` ni `disabled`) a été extraite dans une fonction partagée (`hasUnsavedField()`) et appliquée à l'identique sur `Échap`. Comportement inchangé pour les modales sans champ modifiable (confirmations, listes de choix, fiches en lecture seule) : `Échap` les ferme normalement. Pour les modales avec un champ modifiable, `Échap` ne ferme plus la modale (même animation "nudge" que le clic en dehors) — seule une action explicite (bouton Fermer/Annuler) le fait désormais, ce qui protège une saisie en cours contre une fermeture accidentelle au clavier de la même façon qu'elle l'était déjà contre un clic accidentel en dehors.
...
### LOT 4A — Requêtes ciblées — **Terminé**
...
**Statut (21/09/2026)** : **Terminé.** TODO-009B est Terminé : mutualisation des abonnements Firestore généralisée à `tasks`/`projects`/`followUps`, transparente pour les 8 vues appelantes (aucune n'a eu besoin d'être modifiée). TODO-009A est réalisé pour tout ce qui reste effectivement à sa charge dans ce lot : les volets `tags.js` (conversion à `listWhere`) et `linkedItems.js#renderLinkedSection` (lecture directe + chargement différé des données auxiliaires) sont faits, versionnés et testés ; le volet `usageEvents` (ses deux solutions proposées par la roadmap se sont révélées plus risquées que ce qu'elle annonçait) a été formellement sorti de TODO-009A et repris par **TODO-033**, backlog, non affectée à un lot — même mécanique que TODO-006/TODO-028 en LOT 1. Une découverte hors périmètre (4 autres appelants du même motif `fetchBundle()`/`resolveRef()`) a été signalée sans être corrigée, journalisée sous **TODO-034**, également backlog, non affectée à un lot. Ni TODO-033 ni TODO-034 ne font donc partie du périmètre de LOT 4A. Voir TODO-009A/TODO-009B en section 5 pour le détail complet.

Tests écrits (`tests/unit/lot4a-subscribe-mutualization.spec.js`, `tests/unit/lot4a-tags-listwhere.spec.js`, TEST-013 décliné) : **confirmés par un passage réel du workflow GitHub Actions le 21/09/2026 — suite complète 27/27 verte**, ces deux nouveaux fichiers inclus, sans régression sur les tests des lots précédents. **Confirmation de Charles-Henri (21/09/2026)** : lot validé, TODO-009A/TODO-009B formalisés comme ci-dessus (le volet non réalisé de TODO-009A explicitement hors périmètre de ce lot via TODO-033/TODO-034), LOT 4A passe donc à **Terminé**. Ce lot n'enchaîne pas automatiquement sur LOT 4B, en attente d'une instruction explicite de Charles-Henri pour le démarrer.

**Incident signalé après livraison (21/09/2026)** : Charles-Henri a d'abord remonté deux erreurs au chargement (`SyntaxError` sur un export de `tasks.js`, `Cache.put() encountered a network error`). Deux causes distinctes, aucune liée au code de LOT 4A lui-même :

1. Diagnostic initial : `CACHE_NAME` (`sw.js`) n'avait pas été incrémenté depuis le 15/09/2026 alors que LOT 1, LOT 2, LOT 3 et LOT 4A ont chacun modifié des fichiers précachés — la classe de bug que ce même fichier documente pourtant en détail comme devant être évitée systématiquement. Corrigé hors périmètre de ce lot (`sw.js` n'est concerné par aucun TODO de LOT 4A) : `CACHE_NAME` incrémenté, et `js/components/formValidation.js` (LOT 1) ajouté à `APP_SHELL` où il manquait depuis sa création (même classe d'oubli, détectée à cette occasion par comparaison exhaustive). Un fichier mort supplémentaire trouvé au passage (`js/services/storage-local.js`, jamais réellement supprimé malgré un commentaire de `sw.js` affirmant le contraire) a été journalisé sans être corrigé, voir **TODO-035**. Voir le commentaire détaillé directement dans `sw.js` pour l'analyse complète.
2. Cause réelle de l'erreur persistante (l'incrément de `CACHE_NAME` seul ne l'a pas résolue, y compris en navigation privée, donc sans aucun cache préexistant) : Charles-Henri avait déposé manuellement la mauvaise version d'un fichier sur son dépôt GitHub lors de l'application des livrables de LOT 4A. Confirmé par Charles-Henri après redépôt du bon fichier : l'application recharge normalement et la suite de tests passe 27/27. Aucune anomalie de code n'est donc en cause dans cet incident, au-delà du vrai bug de cache de Service Worker corrigé au point 1 ci-dessus.

### LOT 4B — Écritures ciblées et rétention — **Terminé**
...
**Statut (21/09/2026)** : **Terminé.** TODO-010 est Terminé : primitives d'écriture ciblée ajoutées à `storage.js` (`appendToArray`, `setFields`), réutilisant la file de sérialisation existante (garantie CODE-021 préservée), et appliquées aux mutations additives de `tasks.js`/`projects.js`/`followups.js`. TODO-011 est réalisé pour tout ce qui reste effectivement à sa charge dans ce lot : un point bloquant (`usageEvents` interdit techniquement toute suppression via sa règle Firestore actuelle) a été soumis à Charles-Henri avant toute implémentation, plutôt que tranché unilatéralement ; **décision de Charles-Henri (21/09/2026)** : documenter la politique de rétention sur les 3 collections concernées pour ce lot, et reporter l'implémentation de la purge assistée elle-même (et la question `firestore.rules` d'`usageEvents`) — le volet non réalisé a été formellement sorti de TODO-011 et repris par **TODO-036**, backlog, non affectée à un lot — même mécanique que TODO-009A/TODO-033 en LOT 4A. Une découverte hors périmètre (le même `addNote()` non ciblé dans 4 autres modules de domaine non couverts par TODO-010) a été signalée sans être corrigée, journalisée sous **TODO-037**, également backlog, non affectée à un lot. Ni TODO-036 ni TODO-037 ne font donc partie du périmètre de LOT 4B. Voir TODO-010/TODO-011 en section 5 pour le détail complet.

Test écrit (`tests/unit/lot4b-targeted-writes.spec.js`, TEST-006/TEST-007 déclinés) : **confirmé par un passage réel du workflow GitHub Actions le 21/09/2026 — suite complète 32/32 verte**, ce nouveau fichier inclus, sans régression sur les tests des lots précédents. **Confirmation de Charles-Henri (21/09/2026)** : lot validé, TODO-010/TODO-011 formalisés comme ci-dessus (le volet non réalisé de TODO-011 explicitement hors périmètre de ce lot via TODO-036/TODO-037), LOT 4B passe donc à **Terminé**. Ce lot n'enchaîne pas automatiquement sur LOT 5, en attente d'une instruction explicite de Charles-Henri pour le démarrer.

### LOT 5 — Documentation de référence — **Terminé**
...
**Statut (21/09/2026)** : **Terminé.** TODO-012, seul problème rattaché à ce lot, est Terminé : `PROJECT_CONTEXT.md` reçoit deux nouvelles sections (§16 « Évolution du schéma des données », §17 « Schéma des collections (référence) »), ajoutées en fin de fichier pour ne renuméroter aucune des sections existantes ni casser les renvois internes (`§6`, `§7`, `§10`...) déjà présents ailleurs dans le document. Le volet vocabulaire de l'objectif ci-dessus reste, comme prévu, hors périmètre de ce lot (porté par TODO-021 seul, non planifié à ce jour). Voir TODO-012 en section 5 pour le détail complet. Documentation pure, aucun fichier applicatif modifié, aucun test automatisé prévu ni applicable — seule une relecture croisée par Charles-Henri était encore attendue (voir "Validation" de TODO-012). **Confirmation de Charles-Henri (21/09/2026)** : lot validé, LOT 5 passe formellement à **Terminé**. Le lot n'a pas enchaîné automatiquement sur LOT 6 : son démarrage a fait l'objet d'une instruction explicite et distincte de Charles-Henri, le même jour.

### LOT 6 — Compléments Inbox actés — **Terminé**
...
**Statut (21/09/2026)** : **Terminé.** TODO-013 et TODO-008 sont désormais tous deux **définitivement clos** (voir chacun en section 5 pour le détail complet). TODO-008 a nécessité un arbitrage explicite de Charles-Henri sur son point 1 (lien InboxItem source ↔ entité créée), initialement bloqué par une ambiguïté d'architecture signalée avant tout code sur ce point précis — arbitrage reçu et appliqué le même jour (nouveau type de référence `InboxSource`, résolu dans `js/components/linkedItems.js`, sans toucher au type `"Kept"` existant ni à l'auto-archivage à 15 jours).

**Premier passage réel du workflow GitHub Actions (21/09/2026)** : 39/42 verts. Les 3 échecs concernaient exclusivement les nouveaux tests de ce lot (jamais le code applicatif ni les tests des lots précédents, qui restent tous verts) — un test unitaire trop large (vérifiait l'absence de tout lien au lieu de l'absence spécifique d'un lien `Project`) et deux tests E2E avec une assertion `getByText` ambiguë (texte affiché deux fois dans la même modale, à la fois en titre et en label de champ — un cas préexistant sur `openKeptItemDetail`, non touché par ce lot, et un cas introduit par la nouvelle fiche `openInboxSourceDetail`, corrigé en renommant son label). Détail complet sous TODO-008, section 5. Corrections apportées et livrées.

**Deuxième passage réel du workflow GitHub Actions (21/09/2026, après les corrections ci-dessus)** : 40/42 verts, 2 échecs — les deux dans `tests/e2e/lot6-inbox-source-link.spec.js`, et là encore des défauts de rédaction des tests, pas du code applicatif : la fiche Tâche étant une fiche à onglets (vague 24, préexistante), sa section "🔗 Lié" est masquée (`hidden`) sous l'onglet "Activité" tant que celui-ci n'est pas sélectionné — les tests, écrits sans en tenir compte, cherchaient cette section sans jamais changer d'onglet ; et un sélecteur `getByRole("button", { name: "Fermer" })` matchait aussi, par sous-chaîne, un bouton sans rapport ("Fermer ce conseil" d'un bandeau d'astuce). Les deux corrigés côté test uniquement (clic sur l'onglet "Activité" avant d'interagir avec la section "🔗 Lié" ; `exact: true` sur le sélecteur "Fermer"). Détail complet sous TODO-008, section 5. Corrections apportées et livrées.

**Troisième passage réel du workflow GitHub Actions (21/09/2026, après les corrections ci-dessus)** : 41/42 verts, 1 échec — la même ambiguïté "Fermer"/"Fermer ce conseil" que corrigée au round précédent, mais sur une seconde occurrence du même sélecteur (premier test, fiche `openInboxSourceDetail`) qui avait été omise par erreur lors de la correction précédente. Corrigée à l'identique (`exact: true`). Aucune modification de code applicatif. Détail complet sous TODO-008, section 5.

**Quatrième passage réel du workflow GitHub Actions (21/09/2026)** : **42/42 verts, confirmé par Charles-Henri.** Le LOT 6 est désormais **définitivement clos** : TODO-008 et TODO-013 sont tous deux terminés et validés en conditions réelles (Firebase emulators + Playwright), sans régression sur les tests des lots précédents. Passage au LOT 7 sur instruction explicite de Charles-Henri, le même jour.

### LOT 7 — Ressources et Calendrier — **Terminé**
...
**Statut (21/09/2026)** : **Terminé (implémentation) — non clos, en attente de vérification manuelle par Charles-Henri.** TODO-014 et TODO-015 sont tous deux terminés au niveau de l'implémentation (voir chacun en section 5 pour le détail complet) ; aucun des deux ne prévoit de test automatisé (Validation "Non déterminé — test manuel" pour les deux), donc pas de passage CI à attendre ici — mais une vérification manuelle en conditions réelles reste due avant de considérer ce lot réellement terminé, exactement comme le prévoit la roadmap pour ces deux TODO. Prérequis TODO-006 déjà satisfait (validation partagée livrée et confirmée en CI depuis le LOT 1).

Un constat notable en cours de lot, signalé plutôt que traité en silence : la partie "validation d'URL" de TODO-014 (COMP-UX-012) s'est révélée déjà entièrement réalisée par TODO-006 — non refaite, pour éviter une duplication opportuniste d'un travail déjà livré et validé. La partie "cohérence de liaison Ressource↔Tâche/Projet" (COMP-UX-014) avait d'abord été traitée à la lettre de la Solution, avec pour effet de bord un doublon d'affichage (Ressource visible dans "📎 Ressources" ET "🔗 Lié" après un "+ Créer et lier") signalé à Charles-Henri plutôt que tranché unilatéralement. **Décision de Charles-Henri (21/09/2026)** : « je ne veux pas du doublon on garde uniquement le lié et pas ressource » — le chemin "+ Créer et lier" a donc été remis dans son état d'origine (lien générique seul), et COMP-UX-014 reste explicitement non résolu par choix produit assumé.

Sur TODO-015, un filtre Personne avait de même été ajouté au Calendrier puis signalé comme d'utilité limitée (ne filtre en pratique que les Suivis). **Décision de Charles-Henri (21/09/2026)** : « donc on retire le filtre dans calendrier » — le filtre Personne a été intégralement retiré ; seuls les filtres Type et Projet subsistent.

Un incident de production a également été détecté et corrigé pendant ce lot : la première livraison de `calendar.js` contenait une erreur de syntaxe (backticks à l'intérieur d'un commentaire HTML lui-même à l'intérieur d'un template literal JS) non détectée par la vérification locale habituelle (`node --check`) mais provoquant une vraie `SyntaxError` au chargement dans le navigateur, remontée par Charles-Henri puis corrigée. Une méthode de vérification plus rigoureuse (`node --input-type=module --check < fichier.js`) a été identifiée et appliquée en repassage sur tous les fichiers JS touchés dans ce lot et le précédent — voir le détail sous TODO-015, section 5.

Aucun test automatisé écrit, conformément à la roadmap ("Tests nécessaires : Non déterminé") — vérification faite par inspection de code complète des fichiers concernés, l'environnement de rédaction ne pouvant de toute façon pas exécuter l'app ni Playwright (registre npm bloqué).
