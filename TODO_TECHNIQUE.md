...
**Statut (21/09/2026)** : **Partiellement terminé.** Point de prérequis signalé et tranché par Charles-Henri en cours de lot : LOT 0A restait "Préparé — validation finale restante" (comparaison console/déploiement réel non faits) au moment de démarrer ce lot ; décision prise de considérer ce prérequis suffisant (règles versionnées et validées par l'émulateur en LOT 0B) et de poursuivre. Voir le détail par TODO ci-dessus (section 5) : TODO-006 et TODO-007 sont tous les deux **partiellement terminés** — l'essentiel du code est en place et versionné, mais (1) le volet `firestore.rules`/SEC-011 de TODO-006 n'a volontairement pas été fait, une contrainte d'architecture des règles découverte en cours de lot ayant été signalée à Charles-Henri plutôt que traitée seule ; (2) les trois tests prévus (TEST-017, TEST-010, TEST-025) sont écrits mais non exécutés dans cet environnement (registre npm bloqué, même contrainte que LOT 0B) — à confirmer au premier passage réel du workflow GitHub Actions avant de considérer ce lot Terminé.
