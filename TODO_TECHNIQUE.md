...
Non ré-exécutées dans cet environnement après ces corrections (registre npm bloqué), à confirmer par un nouveau passage réel du workflow GitHub Actions — **TODO-008 ne sera considéré comme définitivement clos qu'après cette confirmation**, conformément à l'instruction de Charles-Henri.
...
**Statut (21/09/2026)** : **Terminé (implémentation) — non clos, en attente de confirmation CI.** TODO-013 et TODO-008 sont désormais tous deux **Terminés au niveau de l'implémentation** (voir chacun en section 5 pour le détail complet). TODO-008 a nécessité un arbitrage explicite de Charles-Henri sur son point 1 (lien InboxItem source ↔ entité créée), initialement bloqué par une ambiguïté d'architecture signalée avant tout code sur ce point précis — arbitrage reçu et appliqué le même jour (nouveau type de référence `InboxSource`, résolu dans `js/components/linkedItems.js`, sans toucher au type `"Kept"` existant ni à l'auto-archivage à 15 jours).
